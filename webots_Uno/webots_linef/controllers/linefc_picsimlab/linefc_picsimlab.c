  ...
  while (wb_robot_step(timestep) != -1) {
    ...
    // Retrieve robot position using the sensors.
       ...
       double right_ir_value = wb_distance_sensor_get_value(right_ir);
        ...
        //scaled  range 0 to 5 
        double left_ir_value_s = 4.9;
        double right_ir_value_s = 4.9;
      
        if( (left_ir_value > right_ir_value) && ( 6 < left_ir_value)&&(left_ir_value < 15))
        {
            left_ir_value_s = 0.1;
        }
        else if ((right_ir_value > left_ir_value) && ( 6 < right_ir_value)&&(right_ir_value < 15))
        {
            right_ir_value_s = 0.1;
        }
                                    
        sprintf(cmd,"set apin[23] %5.3f ",left_ir_value_s);
        send_cmd(cmd);
        sprintf(cmd,"set apin[24] %5.3f ",right_ir_value_s);
        send_cmd(cmd);
        
    
//send accelerometer
    sprintf(cmd,"set part[00].in[00] %i",(int)  (((wb_accelerometer_get_values(accelerometer)[0])/9.81)*16385.5/4));
    send_cmd(cmd);
    sprintf(cmd,"set part[00].in[01] %i",(int)  (((wb_accelerometer_get_values(accelerometer)[1])/9.81)*16385.5/4));
    send_cmd(cmd);
    sprintf(cmd,"set part[00].in[02] %i",(int)  (((wb_accelerometer_get_values(accelerometer)[2])/9.81)*16385.5/4));
    send_cmd(cmd);    

//send gyro
    sprintf(cmd,"set part[00].in[03] %i",(int)  ((wb_gyro_get_values(gyro)[0]*180)/3.1416)/64);
    send_cmd(cmd); 
    sprintf(cmd,"set part[00].in[04] %i",(int)  ((wb_gyro_get_values(gyro)[1]*180)/3.1416)/64);
    send_cmd(cmd); 
    sprintf(cmd,"set part[00].in[05] %i",(int)  ((wb_gyro_get_values(gyro)[2]*180)/3.1416)/64);
    send_cmd(cmd); 
   

        send_cmd("dumpr B3");   
        int m1_speed;
        sscanf(buff+6,"%02x",&m1_speed);
        m1_speed/=4;
        send_cmd("get pin[15]");
        int m1_dir;
        sscanf(buff+9,"%i",&m1_dir);
        
        send_cmd("dumpr B3"); //fixme must be B4
        int m2_speed;
        sscanf(buff+6,"%02x",&m2_speed);
        m2_speed/=4;
        send_cmd("get pin[18]");
        int m2_dir;
        ...
        //printf("m1 dir: %i speed: %i\n",m1_dir,m1_speed);
        //printf("m2 dir: %i speed: %i\n",m2_dir,m2_speed);        
       
        
        double left_speed  = (m1_speed * -(2*m1_dir-1) * max_speed) /100.0;
        double right_speed = (m2_speed * -(2*m2_dir-1) * max_speed) /100.0;
